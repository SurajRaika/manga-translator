...
async def select_order(original_image, boxes):
    ...
    async def process_frame(self, detect_result, seg_result, input_frame):
        try:
            ...
            if self.translator and self.ocr and len(to_translate) > 0:
                ...
                order=await select_order(orignal_image,bboxes)

                ocr_results = await self.ocr(list(images))
                rearranged_ocr_results = [ocr_results[i-1] for i in order]

                translation_results = await self.translator(rearranged_ocr_results)
                reordered_translation_results = [x for _, x in sorted(zip(order, translation_results))]


                to_draw = []
                for bbox,translation,color in zip(bboxes,reordered_translation_results,draw_colors):
